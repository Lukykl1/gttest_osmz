...
void gtinit(void)
{
  ...
  gtcur->stats.last_start = get_time();
  gt_started = get_time();
...
bool gtyield(void)
{
  ...
  {
    long run_time = get_time() - gtcur->stats.last_start;
    //printf("DEBUG: switch runtime: %ld now: %ld last_start %ld \n", run_time, get_time(), gtcur->stats.last_start);
    gtcur->stats.last_sleep = get_time();
  ...
  long wait_time = get_time() - gtcur->stats.last_sleep;
  //printf("DEBUG: switch wait_time: %ld now: %ld last_sleep %ld \n", wait_time, get_time(), gtcur->stats.last_sleep);
